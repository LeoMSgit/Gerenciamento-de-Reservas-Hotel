//JLM-Ativ6 - Projeto Hotel
//Leonardo, Matheus, João Alfredo – 18/06/2024
...
void fRealizarCheckIn(int m[20][14], struct stquarto quartos[20][14]) {
    ...
    clearInputBuffer(); // Limpa o buffer após scanf
    ...
    if (quartos[andarQuarto - 1][numeroQuarto - 1].status == '.') {
        ...
        // Validar se a rua não está vazia e não contém números
        int ruaValido = 1;
        ...
        printf("\nDigite o numero da residencia hospede: ");
        ...
        strcpy(quartos[andarQuarto - 1][numeroQuarto - 1].email, novoHospede.email);
...
void fRealizarReserva(int m[20][14], struct stquarto quartos[20][14]) {
    ...
    printf("\nDigite o numero do andar para realizar reserva (0 para retornar ao Menu): ");
    if (scanf("%d", &andarQuarto) != 1) {
        clearInputBuffer();
        printf("Entrada invalida! Por favor, insira um numero.\n");
        return;
    ...
    if (quartos[andarQuarto - 1][numeroQuarto - 1].status == '.') {
        ...
        strcpy(quartos[andarQuarto - 1][numeroQuarto - 1].ender.cidade, novoHospede.ender.cidade);
        strcpy(quartos[andarQuarto - 1][numeroQuarto - 1].ender.uf, novoHospede.ender.uf);
        quartos[andarQuarto - 1][numeroQuarto - 1].status = 'R'; // Reservado

        printf("\nReserva de %s realizado com sucesso, no quarto %d do andar %d!\n\n", novoHospede.nome, numeroQuarto, andarQuarto);
...
void fInformacoesHospede(int m[20][14], struct stquarto quartos[20][14]) {
	...
	if ((quartos[andarQuarto - 1][numeroQuarto - 1].status == 'O') || (quartos[andarQuarto - 1][numeroQuarto - 1].status == 'R')){
		...
		printf("\nQuarto nao esta ocupado ou reservado! Por favor, selecione um quarto ocupado ou reservado para consultar informacoes.\n");
	}
}
